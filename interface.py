"""
The primary GUI interface for the HR Connect with API tool.
...
class Interface:
    """
    A controller for the overall HR Connect with API Application.
    ...
    def __init__(self):
        """
        Initializes the home window of the HR Connect with API app.
        """
        self.root = tk.Tk()
        self.root.title("HR Connect with API for LinkedIn")
    ...
    def open_search_page(self):
        ...
        try:
            ...
            pass
        # Create company entry field
        company_label = tk.Label(filters_frame,
                                text="Companies (Comma Separated):")
        company_label.pack(pady=5)
        company_entry = tk.Entry(filters_frame)
        company_entry.pack()
        try:
            company_entry.insert(0, saved_settings[7]) # index 7 is the company
        except:
            pass
        ...
        search_button = tk.Button(search_page, text="Search", width=20,
                command=lambda: self.from_search_gui(position_entry.get(),
                                     ...
                                     api_entry.get(),
                                     repeat_var.get(),
                                     company_entry.get())) #added company_entry
    ...
    def from_search_gui(self, positions, locations,
                        exp_op, exp_num, output, api_key, repeat, companies):
        """
        ...
        Parameter companies: The companies to search for.
        Precondition: companies is a String.
        """
        self.save_search_settings(positions, locations,
                                    exp_op, exp_num, output, api_key, repeat, companies) # save companies
        search = True
        try:
            if locations != "":
            ...
            if companies != "":
                c.COMPANIES = companies.split(', ') #add companies to constant
            else:
                c.COMPANIES = [] #allow companies to be blank
    ...
    def save_search_settings(self, positions, locations, exp_op,
                              exp_num, output, api_key, repeat, companies):
        ...
        Precondition: repeat is 0 or 1.
        """
        settings = [locations, positions, exp_op, exp_num, output, api_key, repeat, companies]
